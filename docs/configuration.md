...
| `dast.active.include_post` | `false` | Whether to inject into POST forms as well as GET query parameters. GET-only is the safe default: a POST is more likely to change state on the target. |
